 ...
 * exactmetrics-setup-wizard-link class.
 *
 * The onboarding URL (with its short-lived onboarding_key) is pre-generated
 * server-side at page render and localized as window.exactmetrics.wizard_url,
...
(function () {
			...
			.then(function (response) {
				if (response && response.success && response.data && response.data.wizard_url) {
					...
					if (window.exactmetrics && typeof window.exactmetrics === 'object') {
						window.exactmetrics.wizard_url = url;
					}
	...
	function syncRenderedLinks(url) {
		var links = document.querySelectorAll('a.exactmetrics-setup-wizard-link[href]');
	...
	document.addEventListener('click', function (event) {
		var link = event.target.closest('.exactmetrics-setup-wizard-link');
		if (!link) {
			return;
		}
