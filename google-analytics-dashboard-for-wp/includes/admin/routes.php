...
class ExactMetrics_Rest_Routes {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_exactmetrics_vue_dismiss_promo', array( $this, 'dismiss_promo' ) );
	}
	...
	public function get_license() {
		...
		$can_view_key = current_user_can( 'exactmetrics_save_settings' );

		$site_license    = array(
			'key'         => $can_view_key ? ExactMetrics()->license->get_site_license_key() : '',
			...
			'expiry_date' => ExactMetrics()->license->get_site_license_expiry_date(),
			'is_invalid'  => ExactMetrics()->license->site_license_invalid(),
			'is_agency'   => ExactMetrics()->license->site_is_agency(),
		);
		$network_license = array(
			'key'         => $can_view_key ? ExactMetrics()->license->get_network_license_key() : '',
			...
			'expiry_date' => ExactMetrics()->license->get_network_license_expiry_date(),
			'is_invalid'  => ExactMetrics()->license->network_license_invalid(),
	...
	public function get_settings() {
		...
		// exactmetrics_update_option(), which deletes options with an empty value.
		...
		if ( ! current_user_can( 'exactmetrics_save_settings' ) ) {
			foreach ( exactmetrics_get_sensitive_settings() as $sensitive_setting ) {
	...
	public function get_addons( $is_onboarding ) {
		...
		$parsed_addons['rewardswp'] = array(
			...
			'icon'      => plugin_dir_url( EXACTMETRICS_PLUGIN_FILE ) . 'assets/images/plugins/plugin-rewardswp.png',
			'title'     => 'RewardsWP',
			'excerpt'   => __( 'Build a powerful loyalty rewards program for your WooCommerce store and turn one-time buyers into loyal customers.', 'google-analytics-dashboard-for-wp' ),
		...
		$parsed_addons['universally'] = array(
			...
			'icon'      => '',
			'title'     => 'Universally',
			'excerpt'   => __( 'Automatically translate your website into 110+ languages with AI to reach a global audience and grow international traffic.', 'google-analytics-dashboard-for-wp' ),
	...
	public function update_manual_v4() {
		...
		if ( ! empty( $_REQUEST['isnetwork'] ) && sanitize_text_field( wp_unslash( $_REQUEST['isnetwork'] ) ) ) {
			if ( ! current_user_can( 'manage_network_options' ) ) {
				wp_send_json_error( array(
					'error' => esc_html__( 'You do not have permission to update network settings.', 'google-analytics-dashboard-for-wp' ),
				) );
			}
			define( 'WP_NETWORK_ADMIN', true );
		}
	...
	public function get_report_data() {
		...
		if ( ! current_user_can( 'exactmetrics_view_dashboard' ) ) {
			$message = sprintf(
				/* translators: %1$s: Opening link tag, %2$s: Closing link tag. */
				esc_html__( 'Oops! You do not have permissions to view ExactMetrics reporting. Please check with your site administrator that your role is included in the ExactMetrics permissions settings. %1$sClick here for more information%2$s.', 'google-analytics-dashboard-for-wp' ),
		...
		if ( ! $site_auth && ! $ms_auth ) {
			// `exactmetrics_get_onboarding_url()` already builds the correct
		...
		if ( exactmetrics_is_pro_version() && ! ExactMetrics()->license->license_can( $report->level ) ) {
			...
			ExactMetrics_Report_Views::maybe_record( $report->name );
	...
	public function check_popular_posts_report() {
		...
		if ( exactmetrics_is_pro_version() && ! ExactMetrics()->license->license_can( $report->level ) ) {
			...
			// feeds CS decisions where "opened a report they cannot open" is worse than
			// no data at all. Still above get_data(), so cache hits keep counting.
			ExactMetrics_Report_Views::maybe_record( $report_name );
	...
	public function dismiss_promo() {
		...
		if ( ! current_user_can( 'exactmetrics_view_dashboard' ) ) {
			wp_send_json_error( array( 'message' => esc_html__( 'You do not have permission to do this.', 'google-analytics-dashboard-for-wp' ) ) );
		...
		if ( empty( $promo_id ) ) {
			wp_send_json_error( array( 'message' => esc_html__( 'Missing promo id.', 'google-analytics-dashboard-for-wp' ) ) );

			return;
		}

		$dismissed = exactmetrics_get_dismissed_promos();

		if ( ! in_array( $promo_id, $dismissed, true ) ) {
			$dismissed[] = $promo_id;
			update_user_meta( get_current_user_id(), 'exactmetrics_dismissed_promos', $dismissed );
		}
	...
	public function get_overview_bundle() {
		...
		$is_pro_version = exactmetrics_is_pro_version();
		if ( ! $is_pro_version || ExactMetrics()->license->license_can( $overview_report->level ) ) {
			ExactMetrics_Report_Views::maybe_record( 'overview' );
		}
		if ( ! $is_pro_version || ExactMetrics()->license->license_can( $site_summary_report->level ) ) {
			ExactMetrics_Report_Views::maybe_record( 'site_summary' );
		}
