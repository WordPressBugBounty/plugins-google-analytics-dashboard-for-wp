 ...
 */
if ( ! defined( 'EXACTMETRICS_AI_CHARLIE_CACHE_EXPIRY' ) ) {
	define( 'EXACTMETRICS_AI_CHARLIE_CACHE_EXPIRY', 315360000 );
}
 ...
 */
function exactmetrics_ai_charlie_cache_group( $user_id = 0 ) {
	if ( ! $user_id ) {
 ...
 */
function exactmetrics_ai_charlie_save_chat() {
	check_ajax_referer( 'mi-admin-nonce', 'nonce' );

	if ( ! current_user_can( 'exactmetrics_view_dashboard' ) ) {
		wp_send_json_error( array( 'message' => __( 'You do not have permission.', 'google-analytics-dashboard-for-wp' ) ) );
	}
	...
	if ( empty( $chat ) || empty( $chat['id'] ) || empty( $chat['messages'] ) ) {
		wp_send_json_error( array( 'message' => __( 'Invalid chat data.', 'google-analytics-dashboard-for-wp' ) ) );
	}

	$chat_id = sanitize_text_field( $chat['id'] );
	$group   = exactmetrics_ai_charlie_cache_group();
	$key     = 'chat_' . $chat_id;
	...
	$existing = exactmetrics_cache_get( $key, $group );
	$pinned   = false;
	...
	$result = exactmetrics_cache_set( $key, $data, $group, EXACTMETRICS_AI_CHARLIE_CACHE_EXPIRY );

	if ( false === $result ) {
		wp_send_json_error( array( 'message' => __( 'Failed to save chat.', 'google-analytics-dashboard-for-wp' ) ) );
	}

	wp_send_json_success( array( 'id' => $chat_id ) );
}
add_action( 'wp_ajax_exactmetrics_ai_charlie_save_chat', 'exactmetrics_ai_charlie_save_chat' );

/**
 * User meta key for storing the list of pinned conversation UUIDs.
 */
if ( ! defined( 'EXACTMETRICS_AI_CHARLIE_PINNED_META_KEY' ) ) {
	define( 'EXACTMETRICS_AI_CHARLIE_PINNED_META_KEY', 'exactmetrics_ai_charlie_pinned_chats' );
}

/**
 * Maximum number of pinned conversations a user can have.
 */
if ( ! defined( 'EXACTMETRICS_AI_CHARLIE_MAX_PINNED' ) ) {
	define( 'EXACTMETRICS_AI_CHARLIE_MAX_PINNED', 50 );
}
 ...
 */
function exactmetrics_ai_charlie_get_pinned_ids( $user_id = 0 ) {
	if ( ! $user_id ) {
		$user_id = get_current_user_id();
	}
	$ids = get_user_meta( $user_id, EXACTMETRICS_AI_CHARLIE_PINNED_META_KEY, true );
 ...
 */
function exactmetrics_ai_charlie_get_chats() {
	check_ajax_referer( 'mi-admin-nonce', 'nonce' );
	...
	$cache_table = new ExactMetrics_Cache_Table();
	...
	$rows = $wpdb->get_results(
		$wpdb->prepare(
			"SELECT cache_value FROM {$table} WHERE cache_group = %s AND expires_at > NOW() ORDER BY expires_at DESC LIMIT %d OFFSET %d", // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- table name from ExactMetrics_Cache_Table, not user input.
			$group,
			$per_page + 1,
			$offset
		)
...
add_action( 'wp_ajax_exactmetrics_ai_charlie_get_chats', 'exactmetrics_ai_charlie_get_chats' );
 ...
 * so no full-table scan is needed. Hard cap: EXACTMETRICS_AI_CHARLIE_MAX_PINNED.
 */
function exactmetrics_ai_charlie_get_saved_chats() {
	check_ajax_referer( 'mi-admin-nonce', 'nonce' );
	...
	$pinned_ids = exactmetrics_ai_charlie_get_pinned_ids();
	...
	$rows = $wpdb->get_results(
		$wpdb->prepare(
			"SELECT cache_value FROM {$table} WHERE cache_group = %s AND cache_key IN ({$placeholders}) AND expires_at > NOW()", // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- table name from ExactMetrics_Cache_Table; placeholders built from array_fill, not user input.
...
add_action( 'wp_ajax_exactmetrics_ai_charlie_get_saved_chats', 'exactmetrics_ai_charlie_get_saved_chats' );
 ...
 */
function exactmetrics_ai_charlie_load_chat() {
	check_ajax_referer( 'mi-admin-nonce', 'nonce' );
	...
	if ( empty( $chat_id ) ) {
		wp_send_json_error( array( 'message' => __( 'Missing chat ID.', 'google-analytics-dashboard-for-wp' ) ) );
	}

	$group = exactmetrics_ai_charlie_cache_group();
	$key   = 'chat_' . $chat_id;
	$data  = exactmetrics_cache_get( $key, $group );

	if ( false === $data ) {
		wp_send_json_error( array( 'message' => __( 'Chat not found.', 'google-analytics-dashboard-for-wp' ) ) );
	}

	wp_send_json_success( array( 'chat' => $data ) );
}
add_action( 'wp_ajax_exactmetrics_ai_charlie_load_chat', 'exactmetrics_ai_charlie_load_chat' );
 ...
 * Expects POST['chat_id'].
 */
function exactmetrics_ai_charlie_delete_chat() {
	check_ajax_referer( 'mi-admin-nonce', 'nonce' );
	...
	exactmetrics_cache_delete( $key, $group );

	// Remove from pinned user meta if present.
	$user_id    = get_current_user_id();
	$pinned_ids = exactmetrics_ai_charlie_get_pinned_ids( $user_id );
	$pinned_ids = array_values( array_filter( $pinned_ids, function( $id ) use ( $chat_id ) {
		return $id !== $chat_id;
	} ) );
	update_user_meta( $user_id, EXACTMETRICS_AI_CHARLIE_PINNED_META_KEY, $pinned_ids );

	wp_send_json_success();
}
add_action( 'wp_ajax_exactmetrics_ai_charlie_delete_chat', 'exactmetrics_ai_charlie_delete_chat' );
 ...
 */
function exactmetrics_ai_charlie_pin_chat() {
	check_ajax_referer( 'mi-admin-nonce', 'nonce' );
	...
	exactmetrics_cache_set( $key, $data, $group, EXACTMETRICS_AI_CHARLIE_CACHE_EXPIRY );
	...
	if ( $pinned ) {
		if ( ! in_array( $chat_id, $pinned_ids, true ) ) {
			if ( count( $pinned_ids ) >= EXACTMETRICS_AI_CHARLIE_MAX_PINNED ) {
				wp_send_json_error( array( 'message' => __( 'You have reached the maximum number of saved conversations.', 'google-analytics-dashboard-for-wp' ) ) );
				return;
			}
			$pinned_ids[] = $chat_id;
		}
...
add_action( 'wp_ajax_exactmetrics_ai_charlie_pin_chat', 'exactmetrics_ai_charlie_pin_chat' );
