...
class ExactMetrics_Onboarding {
	...
	public function validate_onboarding_request( $request ) {
		...
		if ( ! exactmetrics_can_install_plugins( $onboarding_user_id ) ) {
			return new WP_Error(
	...
	public function store_settings( $request ) {
		if ( ! function_exists( 'exactmetrics_get_addons_data' ) ) {
			require_once EXACTMETRICS_PLUGIN_DIR . 'includes/admin/pages/addons.php';
		}
			...
			foreach ( $plugins as $plugin ) {
				...
				switch ( $plugin_type ) {
					...
					case 'activation':
						...
						exactmetrics_suppress_wpforms_first_run( $plugin['installation_path'] );
						$activated_plugins[] = $plugin['title'];
						break;
				}
				if ( $installed ) {
					activate_plugin( $installed, '', $is_network, true );
					exactmetrics_suppress_wpforms_first_run( $installed );
					$activated_plugins[] = $plugin['title'];
				}
			}
	 ...
	 *
	 * The catalog is sourced from exactmetrics_get_addons(), which returns
	 * the cached licensing API response (transient `_exactmetrics_addons`)
	...
	private function get_addon_download_urls() {
		...
		$catalog = exactmetrics_get_addons();
