...
class ExactMetrics_Notifications {
	...
	public function hooks() {
		...
		add_action( 'wp_ajax_exactmetrics_notification_mark_read', array( $this, 'mark_read' ) );

		add_action( 'wp_ajax_exactmetrics_notification_save', array( $this, 'save_notification' ) );

		add_action( 'wp_ajax_exactmetrics_notification_remind_me', array( $this, 'remind_me' ) );
	...
	public function get() {
		...
		if ( empty( $option['update'] ) || time() > $option['update'] + DAY_IN_SECONDS ) {
			...
			if ( false === get_transient( 'exactmetrics_notifications_updating' ) ) {
				set_transient( 'exactmetrics_notifications_updating', true, MINUTE_IN_SECONDS );
				$this->update();
				delete_transient( 'exactmetrics_notifications_updating' );
				$option = $this->get_option();
			}
		}
	...
	public function remind_me() {
		...
		$snoozed = get_user_meta( $user_id, 'exactmetrics_notifications_snoozed', true );
		if ( ! is_array( $snoozed ) ) {
			$snoozed = array();
		}
		...
		update_user_meta( $user_id, 'exactmetrics_notifications_snoozed', $snoozed );
	...
	public function get_menu_count() {
		...
		if ( $count > 0 ) {
			return '<span class="exactmetrics-menu-notification-indicator update-plugins">' . $count . '</span>';
		}
