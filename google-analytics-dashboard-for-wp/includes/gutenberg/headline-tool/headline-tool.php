...
class ExactMetricsHeadlineToolPlugin {
	...
	function init() {
		...
		add_action( 'wp_ajax_exactmetrics_gutenberg_headline_analyzer_optimize', array( $this, 'ajax_optimize' ) );
	}
	 ...
	 * ExactMetrics_API_Token, the AI service call is made with wp_remote_post,
	...
	function ajax_optimize() {
		// Reuse the Gutenberg headline nonce — already exposed as exactmetrics_gutenberg_tool_vars.nonce.
		check_ajax_referer( 'exactmetrics_gutenberg_headline_nonce', '_ajax_nonce' );

		// Match the token gate: get_token() requires exactmetrics_view_dashboard,
		...
		if ( ! exactmetrics_is_pro_version() || ! ExactMetrics()->license->license_can( 'plus' ) ) {
		...
		if ( '' === $headline ) {
			wp_send_json_error( $this->ai_error( 'validation', __( 'Headline is required.', 'google-analytics-dashboard-for-wp' ), false ), 400 );
		}
	 ...
	 * ExactMetrics_API_Token. Returns either a WP_Error (transport-level
	...
	private function call_ai_suggestions( array $payload, $force_fresh ) {
		if ( $force_fresh ) {
			\ExactMetrics_API_Token::invalidate( is_network_admin() );
		}

		$token_data = \ExactMetrics_API_Token::get_token( is_network_admin() );
		if ( is_wp_error( $token_data ) || empty( $token_data['token'] ) ) {
			return new \WP_Error( 'mi_ai_auth', '', array( 'type' => 'auth' ) );
		}

		$base = apply_filters( 'exactmetrics_ai_chat_api_url', 'https://ai-api.exactmetrics.com/' );
