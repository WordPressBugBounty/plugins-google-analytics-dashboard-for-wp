...
/**
 * Get the plugin text domain for the active ExactMetrics build.
 *
 * @return string
 */
function exactmetrics_get_plugin_textdomain() {
	return exactmetrics_is_pro_version() ? 'exactmetrics-premium' : 'google-analytics-dashboard-for-wp';
}
 ...
 * that ExactMetrics_API_Auth::get_sitei() works when the onboarding URL is
...
function exactmetrics_get_onboarding_url() {
	...
	if ( empty( $auth ) ) {
		...
		$auth = ExactMetrics()->api_auth = new ExactMetrics_API_Auth();
	}
	...
	$can_launch_wizard = exactmetrics_can_install_plugins();
	...
	$params = array(
		...
		'onboarding_key'    => $can_launch_wizard ? exactmetrics_get_onboarding_key() : '',
...
function exactmetrics_get_onboarding_key() {
	...
	$stored = exactmetrics_get_onboarding_key_data();
	$key    = $stored['key'];
	...
	exactmetrics_store_onboarding_key( $key, $user_id, $ttl );
 ...
 * @return void
 */
function exactmetrics_store_onboarding_key( $key, $user_id, $ttl ) {
	set_transient(
		'exactmetrics_onboarding_key',
		array(
			'key'     => (string) $key,
			'user_id' => (int) $user_id,
		),
 ...
 * }
 */
function exactmetrics_get_onboarding_key_data() {
	$stored = get_transient( 'exactmetrics_onboarding_key' );
	...
	if ( is_string( $stored ) && '' !== $stored ) {
		return array(
			'key'     => $stored,
			'user_id' => (int) get_transient( 'exactmetrics_onboarding_user_id' ),
		);
	}
...
function exactmetrics_get_onboarding_user_id() {
	$data = exactmetrics_get_onboarding_key_data();
...
function exactmetrics_get_page_title() {
	...
	$title = __( 'Archives', 'google-analytics-dashboard-for-wp' );

	if ( is_category() ) {
		/* translators: Category archive title. %s: Category name */
		$title = sprintf( __( 'Category: %s', 'google-analytics-dashboard-for-wp' ), single_cat_title( '', false ) );
	} elseif ( is_tag() ) {
		/* translators: Tag archive title. %s: Tag name */
		$title = sprintf( __( 'Tag: %s', 'google-analytics-dashboard-for-wp' ), single_tag_title( '', false ) );
	} elseif ( is_author() ) {
		/* translators: Author archive title. %s: Author name */
		$title = sprintf( __( 'Author: %s', 'google-analytics-dashboard-for-wp' ), '<span class="vcard">' . get_the_author() . '</span>' );
	} elseif ( is_year() ) {
		/* translators: Yearly archive title. %s: Year */
		$title = sprintf( __( 'Year: %s', 'google-analytics-dashboard-for-wp' ), get_the_date( _x( 'Y', 'yearly archives date format', 'google-analytics-dashboard-for-wp' ) ) );
	} elseif ( is_month() ) {
		/* translators: Monthly archive title. %s: Month name and year */
		$title = sprintf( __( 'Month: %s', 'google-analytics-dashboard-for-wp' ), get_the_date( _x( 'F Y', 'monthly archives date format', 'google-analytics-dashboard-for-wp' ) ) );
	} elseif ( is_day() ) {
		/* translators: Daily archive title. %s: Date */
		$title = sprintf( __( 'Day: %s', 'google-analytics-dashboard-for-wp' ), get_the_date( _x( 'F j, Y', 'daily archives date format', 'google-analytics-dashboard-for-wp' ) ) );
	} elseif ( is_tax( 'post_format' ) ) {
		if ( is_tax( 'post_format', 'post-format-aside' ) ) {
			$title = _x( 'Asides', 'post format archive title', 'google-analytics-dashboard-for-wp' );
		} elseif ( is_tax( 'post_format', 'post-format-gallery' ) ) {
			$title = _x( 'Galleries', 'post format archive title', 'google-analytics-dashboard-for-wp' );
		} elseif ( is_tax( 'post_format', 'post-format-image' ) ) {
			$title = _x( 'Images', 'post format archive title', 'google-analytics-dashboard-for-wp' );
		} elseif ( is_tax( 'post_format', 'post-format-video' ) ) {
			$title = _x( 'Videos', 'post format archive title', 'google-analytics-dashboard-for-wp' );
		} elseif ( is_tax( 'post_format', 'post-format-quote' ) ) {
			$title = _x( 'Quotes', 'post format archive title', 'google-analytics-dashboard-for-wp' );
		} elseif ( is_tax( 'post_format', 'post-format-link' ) ) {
			$title = _x( 'Links', 'post format archive title', 'google-analytics-dashboard-for-wp' );
		} elseif ( is_tax( 'post_format', 'post-format-status' ) ) {
			$title = _x( 'Statuses', 'post format archive title', 'google-analytics-dashboard-for-wp' );
		} elseif ( is_tax( 'post_format', 'post-format-audio' ) ) {
			$title = _x( 'Audio', 'post format archive title', 'google-analytics-dashboard-for-wp' );
		} elseif ( is_tax( 'post_format', 'post-format-chat' ) ) {
			$title = _x( 'Chats', 'post format archive title', 'google-analytics-dashboard-for-wp' );
		}
	} elseif ( is_post_type_archive() ) {
		/* translators: Post type archive title. %s: Post type name */
		$title = sprintf( __( 'Archives: %s', 'google-analytics-dashboard-for-wp' ), post_type_archive_title( '', false ) );
	} elseif ( is_tax() ) {
...
function exactmetrics_tools_copy_url_to_prettylinks() {
	...
	$exactmetrics_reference = isset( $_GET['exactmetrics_reference'] ) ? sanitize_text_field( wp_unslash( $_GET['exactmetrics_reference'] ) ) : '';
 ...
 */
function exactmetrics_get_dismissed_promos( $user_id = null ) {
	if ( empty( $user_id ) ) {
		$user_id = get_current_user_id();
	}

	$dismissed = get_user_meta( $user_id, 'exactmetrics_dismissed_promos', true );
 ...
 * @return bool
 */
function exactmetrics_is_wpforms_plugin( $basename ) {
	return in_array(
		$basename,
		array(
 ...
 * suppression below is scoped to activations ExactMetrics initiated. A user who
 ...
 * @return void
 */
function exactmetrics_suppress_wpforms_first_run( $basename ) {
	if ( ! exactmetrics_is_wpforms_plugin( $basename ) ) {
		return;
	}
	...
	update_option( 'exactmetrics_suppress_wpforms_first_run', 1 );
}
...
add_action( 'admin_init', 'exactmetrics_disable_wpforms_first_run_redirect', 11 );
/**
 * Disarm WPForms' first-run redirect after ExactMetrics installs it.
 *
 ...
 * @return void
 */
function exactmetrics_disable_wpforms_first_run_redirect() {
	...
	if ( ! get_option( 'exactmetrics_suppress_wpforms_first_run' ) ) {
		return;
	}
	...
	if ( get_option( 'wpforms_setup_wizard_initial_version' )
		...
		delete_option( 'exactmetrics_suppress_wpforms_first_run' );
	}
}
