...
class ExactMetrics_Email_Summaries {
	 ...
	 * @var string
	 */
	const ICONS_URL = 'https://www.exactmetrics.com';
	...
	public function get_header_image() {
		...
		$old_value = apply_filters_deprecated(
			...
			__( 'This filter is deprecated. Use exactmetrics_email_header_logo instead.', 'google-analytics-dashboard-for-wp' )
		);
	...
	public function set_template_args( $args ) {
		...
		$theme_name = defined( 'EXACTMETRICS_VERSION' ) ? 'exactmetricsv1' : 'exactmetricsv7';
		$icons_url = apply_filters( 'exactmetrics_get_licensing_url', self::ICONS_URL ) . '/wp-content/themes/' . $theme_name . '/assets/newsletter/';
	...
	public function get_summaries() {
		...
		if ( ! empty( $data['success'] ) && ! empty( $data['data'] ) && is_array( $data['data'] ) ) {
			...
			if ( ! $has_stats ) {
				$overview = ExactMetrics()->reporting->get_report( 'overview' );
				if ( $overview && ! ( exactmetrics_is_pro_version() && ! ExactMetrics()->license->license_can( $overview->level ) ) ) {
					$overview_data = apply_filters( 'exactmetrics_vue_reports_data', $overview->get_data( $args ), 'overview', $overview );
