import { k as getMiGlobal, O as getExactMetricsUrl, l as defineStore, N as isNetworkAdmin, p as getUrl } from "./toastStore-nTGs-rGm.js";
...
const fetchLicense = async () => {
  try {
    return await miAjax("exactmetrics_vue_get_license");
  } catch (error) {
...
const verifyLicense = async (key, isNetwork = false) => {
  try {
    return await miAjax("exactmetrics_verify_license", {
      license: key,
...
const validateLicense = async (isNetwork = false) => {
  try {
    return await miAjax("exactmetrics_validate_license", {
      network: isNetwork || void 0
    });
  } catch (error) {
    const support_url = getExactMetricsUrl(
...
const deactivateLicense = async (key, isNetwork = false) => {
  try {
    return await miAjax("exactmetrics_deactivate_license", {
      license: key,
      network: isNetwork || void 0
    });
  } catch (error) {
...
const deactivateExpiredLicense = async (key, isNetwork = false) => {
  try {
    return await miAjax("exactmetrics_deactivate_expired_license", {
      license: key,
      network: isNetwork || void 0
    });
  } catch (error) {
...
const getUpgradeLink = async (key) => {
  try {
    const response = await miAjax("exactmetrics_connect_url", {
      key,
    ...
    throw {
      title: __$1("Error Getting Upgrade Link", "google-analytics-dashboard-for-wp"),
      message: __$1("Empty response from server when trying to get upgrade link.", "google-analytics-dashboard-for-wp"),
