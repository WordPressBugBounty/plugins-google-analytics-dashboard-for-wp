...
import { u as useOverviewReportStore, A as isSampleDataEnabled, b as buildApiFilters } from "../reports-CaiFcwwu.js";
import { c as generateTrafficLandingPagesSample, d as fetchTrafficLandingPagesData } from "./trafficSampleData-B3PeToPS.js";
...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const columns = [
      { key: "landingPage", label: __("Landing Page", "google-analytics-dashboard-for-wp"), sortable: true, linkKey: "landingPageUrl" },
    ...
    function formatLandingRow(dims, vals) {
      ...
      const landingPage = dims[0] != null && String(dims[0]).trim() !== "" ? String(dims[0]) : __("(not set)", "google-analytics-dashboard-for-wp");
      return {
        landingPage,
        landingPageUrl: siteUrl + landingPage,
    ...
    const { rawData, loading, error, reload } = useReport({
      ...
      // Honour the sample-data toggle, matching CountriesReport, PublishersPagesReport
      // and the four other views that pass this. Without it the sample branch only
      // fires for blocked users, so with the toggle on this report fell through to a
      // live request carrying the demo key — a GA property with no landing pages —
      // and rendered its empty state.
      ...
      return openBlock(), createBlock(ReportPageLayout, {
        ...
        table: withCtx(() => [
          createVNode(_sfc_main$1, {
            title: unref(__)("Top Landing Pages", "google-analytics-dashboard-for-wp"),
            columns,
            ...
            loading: unref(loading),
            searchable: "",
            "empty-message": unref(__)("No data currently for the Landing Page report.", "google-analytics-dashboard-for-wp")
