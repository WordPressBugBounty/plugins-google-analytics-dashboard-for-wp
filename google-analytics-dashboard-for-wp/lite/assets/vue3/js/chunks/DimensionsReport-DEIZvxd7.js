import { a1 as storeToRefs, o as openBlock, E as createBlock, D as withCtx, c as createElementBlock, F as Fragment, f as renderList, u as unref, m as computed } from "./toastStore-sH51RcCX.js";
...
import { j as getMiGlobal } from "./ajax-2wfIeviZ.js";
...
async function fetchDimensionsReportData(dateRange, apiFilters = null) {
  ...
  const errorLabel = __("Error loading dimensions report data", "google-analytics-dashboard-for-wp");
...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    function showNoDimensionsModal() {
      ...
      const settingsUrl = `${adminUrl}admin.php?page=exactmetrics_settings#/conversions?section=dimensions`;
      showConfirmModal({
        title: __$1("No Custom Dimensions Configured", "google-analytics-dashboard-for-wp"),
        message: __$1("Please enable at least 1 custom dimension in your ExactMetrics settings to use this report.", "google-analytics-dashboard-for-wp"),
        confirmButtonText: __$1("View Dimensions Settings", "google-analytics-dashboard-for-wp"),
    ...
    const enabledDimensions = computed(() => {
      if (isBlocked.value || isSampleDataEnabled()) {
        return [
          { key: "author", label: __$1("Author", "google-analytics-dashboard-for-wp"), enabled: true },
          { key: "post_type", label: __$1("Post Type", "google-analytics-dashboard-for-wp"), enabled: true },
          { key: "category", label: __$1("Category", "google-analytics-dashboard-for-wp"), enabled: true },
          { key: "tags", label: __$1("Tags", "google-analytics-dashboard-for-wp"), enabled: true },
          { key: "seo_score", label: __$1("SEO Score", "google-analytics-dashboard-for-wp"), enabled: true },
          { key: "logged_in", label: __$1("Logged In", "google-analytics-dashboard-for-wp"), enabled: true },
          { key: "wp_user_id", label: __$1("User ID", "google-analytics-dashboard-for-wp"), enabled: true },
          { key: "focus_keyword", label: __$1("Focus Keyword", "google-analytics-dashboard-for-wp"), enabled: true },
          { key: "publish_time", label: __$1("Publish Time", "google-analytics-dashboard-for-wp"), enabled: true }
        ];
      }
    ...
    function getColumns(dim) {
      return [
        ...
        { key: "views", label: __$1("Views", "google-analytics-dashboard-for-wp"), sortable: true }
      ];
    }
    ...
    function dimEntryToRow(entry, useValues) {
      ...
      const name = dimVal != null && String(dimVal).trim() !== "" ? String(dimVal) : __$1("(not set)", "google-analytics-dashboard-for-wp");
      return {
        label: name,
    ...
    function getDimensionEmptyText(label) {
      return sprintf(
        __$1("No data currently for the %s report", "google-analytics-dashboard-for-wp"),
        label
      );
    }
