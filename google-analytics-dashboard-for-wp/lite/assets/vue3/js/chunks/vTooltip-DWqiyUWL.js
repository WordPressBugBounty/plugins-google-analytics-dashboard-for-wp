const TOOLTIP_CLASS = "exactmetrics-tooltip";
const GAP = 8;
function buildTooltip(content) {
  ...
  arrow.className = "exactmetrics-tooltip-arrow";
  const inner = document.createElement("div");
  inner.className = "exactmetrics-tooltip-inner";
...
function positionTooltip(tooltip, target) {
  ...
  if (top < window.scrollY) {
    ...
    tooltip.classList.add("exactmetrics-tooltip--bottom");
  } else {
    tooltip.classList.remove("exactmetrics-tooltip--bottom");
  }
...
function show(el) {
  ...
  if (!tooltip) {
    ...
    tooltip.querySelector(".exactmetrics-tooltip-inner").innerHTML = content;
  }
