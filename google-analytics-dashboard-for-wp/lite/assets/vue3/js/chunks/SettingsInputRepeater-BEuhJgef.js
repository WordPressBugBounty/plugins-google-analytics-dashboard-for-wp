import { o as openBlock, c as createElementBlock, F as Fragment, f as renderList, t as toDisplayString, s as createCommentVNode, a as createBaseVNode, i as normalizeClass, w as withDirectives, v as vModelText, B as withModifiers, u as unref, m as computed, j as ref } from "./toastStore-nTGs-rGm.js";
import { q as sanitizeHtml } from "./ajax-BSXoXmkG.js";
...
const _hoisted_1 = { class: "exactmetrics-settings-input-repeater" };
const _hoisted_2 = {
  key: 0,
  class: "exactmetrics-settings-input-repeater-labels exactmetrics-settings-input-repeater-row"
};
const _hoisted_3 = ["textContent"];
const _hoisted_4 = { class: "exactmetrics-settings-input-repeater-row" };
...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const text_add_path = props.text_add ? props.text_add : __("Add Another Link Path", "google-analytics-dashboard-for-wp");
    ...
    const button_class = computed(() => {
      ...
      if (disabled.value) cls += " exactmetrics-button-disabled";
      return cls;
    });
    const rows = computed({
      get() {
      ...
      for (const index in rows.value) {
        for (const structure in props.structure) {
          if (rows.value[index][props.structure[structure].name] === "") {
            has_errors.value[index] = sprintf(__("%s can't be empty.", "google-analytics-dashboard-for-wp"), "<strong>" + props.structure[structure].label + "</strong>");
            break;
          }
          ...
          if (props.structure[structure].prevent_duplicates) {
            ...
            if (names.length !== unique.length) {
              has_errors.value[index] = __("Duplicate values are not allowed.", "google-analytics-dashboard-for-wp");
            }
          }
        }
      }
    ...
    function errorClass(index, name) {
      ...
      if (has_errors.value[index]) return "exactmetrics-input-error";
      return "exactmetrics-input-valid";
    }
    return (_ctx, _cache) => {
